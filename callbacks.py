...
class Model_checkpoint():
    def __init__(self, model_name, recordStep=50, save_best_only = False, lastTime=1000):
        self.min_loss = None
    ...
    def on_epoch_end(self, val_loss, nth_epoch, saver, sess, *args, **kwargs):
        if self.min_loss is None or val_loss < self.min_loss and self.save_best_only:
            self.min_loss = val_loss
            saver.save(sess, 
                       self.model_name + "_{0}".format(nth_epoch) +'.ckpt')
#         if not self.save_best_only:
#             saver.save(sess, 
#                        self.model_name + "_{0}".format(nth_epoch) + '.ckpt',
#                        global_step=nth_epoch)
        if nth_epoch >= self.lastTime-1:
            saver.save(sess, 
                       self.model_name + "_{0}".format(nth_epoch) + '.ckpt',
                       global_step=nth_epoch)
#         if nth_epoch%self.recordStep==0:
#             path = os.path.join(self.model_name, "Newest")
#             saver.save(sess, 
#                        path + "_{0}".format(nth_epoch) + '.ckpt',
#                        global_step=nth_epoch)
